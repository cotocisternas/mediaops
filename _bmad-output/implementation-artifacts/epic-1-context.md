...
- 1.3 shipped DesiredState / Plan / Action (what Epic 4 applies). Original 1.3 also promised `core::jobs` + `title_index`/`jobs` sqlite; that remainder is the 1.3 spec split, not a closed 1.3 AC. Remaining Action variants exist as types but are not applied until Epics 5–7.
- 1.4’s Control / Transfer contract is what Epic 2 bound. Epic 1 itself was types and conversions only.
- `probes` landed with Epic 2 (`user_version = 1`). `holds_decisions` lands with the holds inbox (Epic 6). `title_index` / `jobs` still wait on the 1.3 split remainder.

...
- `probes` landed with Epic 2 (`user_version = 1`). `title_index` / `jobs` (with `title_id`) are `user_version = 3`. `holds_decisions` lands with the holds inbox (Epic 6).

...
- Identity is TitleId (kind + TMDB / TVDB / MBID), not path. Local sqlite maps TitleId → path → inode / BLAKE3 so identity survives rename. Music remasters: MBID, not folder year (Relayer 1974 vs Relayer.(2013)).
- PathSchema is the single writer for names, install gate, lint, agent proposals, and generated docs. `parse(render(id)) == id`. Year lives in the folder and the file the same way. Scene-tag strip includes REPACJ, REPACK, PROPER. Spaces refused. Explicit reject bins include needs-split and needs-year.

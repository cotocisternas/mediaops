...
impl std::fmt::Display for AppError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Usage(message) | Self::Policy(message) => write!(f, "{message}"),
...
async fn run(cli: Cli) -> Result<(), AppError> {
    match cli.command {
        ...
        Some(Command::Seedbox(SeedboxArgs {
            ...
            match bootstrap::bootstrap(args).await {
                Ok(report) => {
                    let line = bootstrap::render_report(cli.json, &report)
                        .map_err(|e| AppError::Runtime(anyhow!(e)))?;
                    write_stdout(&line)
                }
                Err(bootstrap::BootstrapError::NeedsConfirm(report)) => Err(AppError::Policy(
                    bootstrap::BootstrapError::NeedsConfirm(report).to_string(),
                )),
                Err(err) => {
